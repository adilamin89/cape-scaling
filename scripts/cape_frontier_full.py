...
print(f"n={len(MODELS)}: r={r:.3f}, p={p:.6f}")
print(f"Fit: GPQA = {sl:.2f}·SWE + {ic:.1f}")
print()
print(f"{'Model':24} SWE   GPQA     h    Family")
print("-"*65)
for (name,sv,gv,fam), h in zip(MODELS, h_vals):
    hs = ("+" if h>=0 else "") + f"{h:.1f}"
    print(f"  {name:22} {sv:4.1f}  {gv:4.1f}  {hs:6}  {fam}")

# Within-family γ₁₂
print("\nWithin-family γ₁₂ trajectories:")
for fam in ["Anthropic","OpenAI","Google"]:
    ...
    for i in range(len(mods)-1):
        ...
        if abs(ds) > 0.5:
            ...
            print(f"    {mods[i][0][:18]:18}→{mods[i+1][0][:18]:18} γ₁₂={g12:+.2f} [{tag}]")
