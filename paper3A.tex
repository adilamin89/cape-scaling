...
The 88B scale ($d_{\rm eff} \to 1$) and the 130B scale ($\det(H) \to 0$) measure different events; their consistency validates the framework (Appendix~A9).
...
The two-field theory correctly predicts its own breakdown ($d_{\rm eff} \to 1$ means the description has collapsed to a single mode), and the \emph{rise} of $d_{\rm eff}$ back to 1.75 at frontier scale is the third axis activating---exactly the event the scope boundary was designed to detect (Appendix~A9; full calculation in Appendix~A11).
\end{enumerate}
...
The feedback terminates at the phase-locked angle: the SFEE slope $A = 0.629 = \sin(38.8^\circ)$---linking the susceptibility measurement and the eigenvector rotation into a single identity (Appendix~A2).

\textbf{Mixed-phase regime.}
The transition is Type-II ($\lambda/\xi \approx 0.767 > 1/\sqrt{2}$; Appendix~A7): between $N_c$ and $N_{c,2}$, training-recipe perturbations can pin local tax regions inside the cooperative phase.
...
Above $N_c$, scaling and alignment are cooperative---confirmed independently for HS-TQA (base models) and SWE-GPQA (frontier, $r=+0.85$ across 20 models)---with no fixed point observed up to 141B.
...
The four phases are classified by \emph{coupling sign}, not model era or lab: Tax if $\gamma_{12}<0$, Bonus if $\gamma_{12}>0$, regardless of generation. ``Phase~4'' is the scale where HellaSwag and TruthfulQA saturate above 90\% and the relevant capability pair shifts to autonomous coding (SWE-bench) versus scientific reasoning (GPQA Diamond).
...
Phase~4  & $>$70B   & $>0$   & $1.75\to 2.5$      & ---              & ---            & SWE, GPQA, IFEval \\
\bottomrule
\end{tabular}
\end{table}
...
The three major labs are approaching $N_{c,3}$ from different directions in capability space, each with a distinctive $h$-field signature:
\emph{Google} (Gemini~2.5~Pro $\to$ 3~Flash $\to$ 3~Pro $\to$ 3.1~Pro) shows the tightest SWE convergence ($76$--$81\%$) with consistently high $h > +4$---pushing reasoning while coding saturates, riding the $N_{c,3}$ transition naturally.
\emph{OpenAI} (GPT-4o $\to$ 5 $\to$ 5.1 $\to$ 5.2~Pro $\to$ 5.4) has $h > 0$ at every generation: systematic ascent along the cooperative curve, with $h$ growing from $+1.9$ to $+5.3$.
GPT-5.4 ($h = -1.5$) is the first OpenAI model to dip below the line---possibly the beginning of a coding-specialist excursion analogous to Anthropic's Sonnet~4.6.
\emph{Anthropic} oscillates between coding vortices and recovery: Haiku~4.5 ($h = -11.6$) and Sonnet~4.6 ($h = -13.4$) dip sharply; Opus~4.6 ($h = +2.8$) recovers.
This oscillation is itself a measurement---it reveals that Anthropic's training pipeline alternates between coding-emphasis and balanced training recipes, while Google and OpenAI have maintained monotonically reasoning-rich trajectories.
The $h$-field rotation from SWE-GPQA to GPQA-IFEval is visible in all three.
...
\begin{table}[ht]
\centering\small
\caption{Per-lab $h$-field diagnostic and training-recipe recommendation (March 2026).
$\bar{h}$: mean $h$-field across all models from each lab in Table~\ref{tab:frontier}.
Recommendation follows directly from the cooperative regression.}
\label{tab:lab_diagnostic}
\begin{tabular}{llrrl}
\toprule
Lab & Best model & $\bar{h}$ & Position & Primary lever \\
\midrule
Anthropic & Opus~4.6     & $-4.3$  & On manifold$^\dagger$     & Scale uniformly \\
Google    & Gemini~3.1~Pro & $+6.5$ & Reasoning-rich  & Invest in agentic coding \\
OpenAI    & GPT-5.4      & $+2.3$  & On manifold     & Scale uniformly \\
DeepSeek  & V3.2         & $-3.5$  & On manifold     & Scale uniformly \\
Alibaba   & Qwen3.5-397B & $+5.7$  & Reasoning-rich  & Invest in agentic coding \\
Moonshot  & Kimi~K2.5    & $+2.3$  & On manifold     & Scale uniformly \\
MiniMax   & M2.5         & $-3.0$  & On manifold     & Scale uniformly \\
\bottomrule
\end{tabular}
\end{table}
...
\noindent\textbf{Caveats.} The ``Primary lever'' column follows mechanically from the cooperative regression and is not a normative recommendation---labs have strategic considerations beyond two benchmark scores. The sample per lab ranges from 1 to 7 models; labs with fewer data points (DeepSeek, Alibaba, MiniMax) have higher uncertainty in $\bar{h}$. The classification is a snapshot (March 2026) and will shift as new models are released.
...
The framework has already survived several internal consistency checks: OLMo at $\gamma_{12}=0.000$ with zero free parameters, Llama-2 holdout at 5.6\% MAE, 12 diagnostics converging on one coupling structure, and monotone signal strengthening across three dataset expansions (8 $\to$ 26 $\to$ 44 models).
...
The cooperative fit provides a forward prediction: $\text{GPQA}_{\rm pred} = 0.79\cdot\text{SWE} + 24.7 + h_{\rm lab}$ (validated at 6.9\% MAE across 4 held-out labs).
Furthermore, the running coupling $\gamma_{12}(N)$ follows a kink-soliton profile---the instanton (minimum-action trajectory) of the double-well Ginzburg-Landau potential---with width $\Delta(\log_{10}N) \approx 1$ decade centred on $N_{c,1}$.
Training recipes that deviate from this trajectory pay an action penalty exponential in the deviation; detailed predictions for individual labs are given in Paper~3B.
...
A fully worked example---diagnosing a hypothetical frontier release using all three CAPE steps ($h$-field, isocline boundary, trajectory coupling)---is given in Appendix~C of the Supplementary Material.
...
The interactive dashboard at \href{https://adilamin89.github.io/cape-scaling}{\texttt{adilamin89.github.io/cape-scaling}} implements the full protocol: enter any model's scores to get phase classification, $\gamma_{12}$, $d_{\rm eff}$, $h(\mathcal{D})$, and phase-specific recommendations.
...
{\Large\textbf{Supplementary Material}}\\[4pt]
