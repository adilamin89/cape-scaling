...
The symmetry structure expands at each transition (Ising $\to$ $U(1)$ $\to$ $O(3)$), with the $\sqrt{}$ isocline phase classifier validated across all three regimes.
The framework predicts $N_{c,4}$ at $\sim$200--400B effective parameters and its own successor: at that scale, the residual of the current description is the input signal for a four-field extension.
...
Among the four models with IFEval data: Opus~4.6 ($94.0 > \sqrt{0.97 \times 91.3} = 9.41$, rescaled: $94.0 > 88.6$\footnote{The $\sqrt{}$ classifier operates on fractional scores: $\sqrt{0.97 \times 0.913} = 0.941$, i.e.\ $94.1\%$. Opus~4.6 at $94.0\%$ is right at the boundary---exactly where a model at the $N_{c,3}$ transition should sit.}), Kimi~K2.5 ($94.0$ vs.\ $\sqrt{0.97 \times 87.6} \approx 92.1\%$, cooperative), MiniMax~M2.5 ($87.5$ vs.\ $\sqrt{0.97 \times 85.0} \approx 90.8\%$, \emph{below}---consistent with a coding-specialist $h < 0$).
...
\textbf{The $N_{c,3}$ forward prediction.}
The same logic extends to the three-field regime.
Given a lab's GPQA score and its $N_{c,3}$ $h$-field, the predicted IFEval is
$\text{IFEval}_{\rm pred} = 0.97\cdot\text{GPQA} + 6.8 + h_{3,\rm lab}$
(Eq.~\ref{eq:nc3_coupling}).
Among the four models with data: Opus~4.6 ($h_3 = 94.0 - (0.97 \times 91.3 + 6.8) = -1.4$, near baseline),
Kimi~K2.5 ($h_3 = 94.0 - (0.97 \times 87.6 + 6.8) = +2.3$, instruction-rich),
Qwen~3.5-397B ($h_3 = 92.6 - (0.97 \times 88.4 + 6.8) = +0.1$, near baseline),
MiniMax~M2.5 ($h_3 = 87.5 - (0.97 \times 85.0 + 6.8) = -1.8$, instruction-deficient).
The scatter is small ($|h_3| < 2.5$ for three of four), confirming the cooperative coupling at $N_{c,3}$ is tight.
MiniMax's marginally negative $h_3$ is consistent with its position below the $\sqrt{}$ isocline boundary (\S\ref{sec:Nc2_section}).
...
\textbf{The symmetry cascade as a group-subgroup chain.}
The progression Ising $\to$ $U(1)$ $\to$ $O(3)$ $\to$ $O(4)$ is not merely an analogy---it is a group-subgroup chain of the same type that governs second-order phase transitions in condensed matter.
In heavy-fermion superconductors, the pairing symmetry descends from the point group through a subgroup chain ($D_{6h} \supset D_{2h} \supset C_{2v}$), with each symmetry-lowering transition activating new order-parameter components~\cite{amin2020}.
In superfluid $^3$He, the order parameter lives in $SO(3)_L \times SO(3)_S \times U(1)_\phi$, and successive transitions (A-phase $\to$ B-phase) activate additional components of this space---the classification of allowed topological defects changes at each step (point defects in $^3$He-A, no stable point defects in $^3$He-B).
The CAPE cascade follows the same logic:
at $N_{c,1}$, only one capability axis is independent ($\mathbb{Z}_2$, Ising);
at $N_{c,2}$, the second axis activates and the order parameter becomes a 2-vector ($U(1)$, supporting vortex excursions like Sonnet~4.6);
at $N_{c,3}$, the third axis activates ($O(3)$, supporting hedgehog defects---point-like disruptions in 3D capability space that cannot be removed by continuous deformation).
The key empirical question for Paper~3B is whether the $N_{c,3}$ transition is first-order (discontinuous jump in $d_{\rm eff}$) or second-order (continuous growth of the third eigenvalue through zero), which determines whether the symmetry-breaking pattern is Landau-allowed or requires fluctuation corrections beyond mean field.
The current data---$d_{\rm eff} = 1.75$ at frontier scale, approaching but not yet reaching $2.0$---is consistent with a continuous (second-order) transition, though the small sample ($n = 4$) prevents a definitive classification.
...
\textit{If you are training at frontier scale ($N_{c,2}$ regime)}: track $\gamma_{12}(\text{SWE}, \text{GPQA})$ across consecutive releases to detect training-recipe excursions before deployment.
...
The cooperative fit provides a forward prediction: $\text{GPQA}_{\rm pred} = 0.79\cdot\text{SWE} + 24.7 + h_{\rm lab}$ (validated at 5.6\% MAE across 4 held-out labs).
\textit{If you are at the $N_{c,3}$ frontier (SWE $> 78\%$)}: SWE-bench is saturating.
Switch to tracking $\gamma_{12}(\text{GPQA}, \text{IFEval})$---this is now the active coupling pair.
The $N_{c,3}$ forward prediction is $\text{IFEval}_{\rm pred} = 0.97\cdot\text{GPQA} + 6.8 + h_{3,\rm lab}$ (Eq.~\ref{eq:nc3_coupling}; preliminary, $n=4$).
If your model's IFEval is below $\sqrt{0.97\cdot\text{GPQA}}$ (the isocline boundary), your training recipe is entering a new tax regime between reasoning and instruction-following---the $N_{c,3}$ analogue of the original alignment tax.
The practical lever at this scale is no longer data curation (the $N_{c,1}$ lever) or coding optimisation (the $N_{c,2}$ lever), but the balance between reasoning depth and instruction compliance---a training-recipe choice visible in the $h_3$-field.
...
The $\sqrt{}$ isocline classifier generalises to every regime: compute the cooperative slope $a/b$ from the active benchmark pair, then test whether $\text{score}_2 > \sqrt{(a/b)\cdot\text{score}_1}$.
This single formula---derived once from the Riccati ODE---works at $N_{c,1}$ (HS-TQA), $N_{c,2}$ (SWE-GPQA), and $N_{c,3}$ (GPQA-IFEval) without modification.
