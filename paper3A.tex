...
\textbf{OPT as a second within-family confirmation.}
The OPT family (Meta, 1.3B--66B)~\cite{zhang2022opt} provides an independent within-family sign flip:
$\gamma_{12}(\text{OPT-2.7B}\to\text{OPT-6.7B}) = -0.906$ (mid-scale 4.7B $\approx N_c$, negative) and
$\gamma_{12}(\text{OPT-6.7B}\to\text{OPT-30B}) = +1.24$ (mid-scale 18B $> N_c$, positive).
This is the first web-trained family \emph{other than Pythia} showing a within-family sign flip, with the crossing consistent with $N_c \approx 3.5$B.
...
\textbf{The messenger field and mean-field comparison.}
...
The measured exponent $\beta=0.40\pm0.08$ deviates dramatically from this baseline---and the L1 test confirms it: adding the mean-field correction \emph{worsens} prediction by $142\times$ (Table~\ref{tab:boost}).
...
Above $N_c$, the relative phase between the two capability fields locks at $\theta^*=38.8^\circ$---this is time-reversal symmetry breaking (TRSB): the cooperative phase has a specific orientation in capability space that is not related to its mirror image by time-reversal.
...
This explains within-family heterogeneity: training-recipe perturbations create \emph{vortex-like} local excursions into the tax phase inside an otherwise cooperative background---topologically stable structures analogous to Abrikosov vortices in Type-II superconductors.
...
The sign change is confirmed across five additional families (Llama-1, Llama-2, OLMo, OPT, Phi), by the architecture probe, and by the 44-model dataset growing from r~$=+0.77$ to $+0.85$ with every addition---never reversing.
...
The Ginzburg number $\mathrm{Gi} = 1.35$ is a measurement, not a caveat.
It tells us the transition is fluctuation-dominated---Type-II rather than Type-I behavior.
...
(i)~\emph{Screening}: above $N_c$, the free energy includes a stabilizing term $-\eta\,\mathrm{Im}(\psi_R^*\psi_C)$ (TRSB footnote, \S\ref{sec:topology}) that screens perturbations---analogous to the Meissner effect.
...
All benchmark data, figure-generation scripts, and the three new analysis scripts from this paper (\texttt{cape\_frontier\_full.py}, \texttt{cape\_architecture\_probe.py}, \texttt{cape\_stiffness.py}) are at \href{https://github.com/adilamin89/cape-scaling}{\texttt{github.com/adilamin89/cape-scaling}}.
