...
Across 31 frontier models (2024--2026), SWE-bench and GPQA Diamond couple cooperatively ($r=+0.85$, $p<10^{-6}$), with training-recipe excursions visible as $h$-field deviations.
...
At frontier scale, SWE-bench and GPQA Diamond show the same cooperative structure ($r=+0.85$, confirmed across 31 models and 8 labs with cross-lab holdout at 6.9\% MAE).
...
We establish this through five independent lines of evidence: (1)~direct coupling measurement across 63 models and 16 families (\S\ref{sec:coupling}); (2)~an ODE discovered by sparse regression that reproduces benchmark dynamics and cross-predicts held-out Llama-2 (\S\ref{sec:ode}); (3)~gradient measurements revealing collective, not independent, parameter scaling (\S\ref{sec:gradients}); (4)~geometric analysis showing the capability manifold restructures at $N_c$ (\S\ref{sec:topology}); and (5)~frontier validation across 31 models from 8 labs, including preliminary evidence for a third-axis transition (\S\ref{sec:Nc2_section}).
...
This is consistent with our Frontier measurement at large $N$ ($d_{\rm eff}\to 1$, deep cooperative regime).
...
We collect benchmark scores from 16 model families (Table~\ref{tab:models}).
...
OPT~\cite{zhang2022opt} & 125M--66B (8) & 31--75 & 29--43 & 23--46 & 25--27 \\
BLOOM~\cite{bloom2022} & 560M--176B (6) & 37--73 & 38--42 & 25--49 & 24--39 \\
Cerebras-GPT & 111M--13B (6) & 27--60 & 38--46 & 20--38 & 25--27 \\
Qwen2.5 & 0.5B--72B (6) & 52--88 & 40--60 & 36--72 & 48--86 \\
\hline
\multicolumn{2}{l}{\textbf{Total: 63 models, 16 families}} \\
\hline
\end{tabular}
\end{table}
...
\textbf{The findings strengthen monotonically with every dataset expansion}---from 8 to 26 to 63 base models, from 10 to 31 frontier models.
...
The gradient measurements in this section tell us \emph{that} parameters are collectively coupled---and the ODE eigenvalues (\S\ref{sec:ode}) tell us \emph{how}: the tax-phase coefficient matrix has one unstable mode ($\lambda = +0.536$), meaning the alignment tax is a dynamical instability that actively pushes TQA and HS apart, not a static constraint.
...
\subsection{Practical guide: $d_{\rm eff}$ and benchmark selection}
...
=======
Above $N_c$, scaling and alignment are cooperative---confirmed independently for HS-TQA (base models) and SWE-GPQA (frontier, $r=+0.85$ across 31 models)---with no fixed point observed up to 141B.
>>>>>>> Stashed changes
...
We measure $\gamma_{12}(\text{SWE}, \text{GPQA})$ across 31 publicly evaluated frontier models from 8 labs
(Table~\ref{tab:frontier}; scores as of March~2026; $h = \text{GPQA}_{\rm measured} - (0.52\cdot\text{SWE} + 45.7)$):
cross-family $r = +0.73$ ($n = 31$, $p < 10^{-5}$; cooperative coupling confirmed),
...
The cooperative slope $0.52$ of the SWE--GPQA fit gives $a_2/b_2 \approx 0.52$,
so the frontier phase classifier is: $\mathrm{GPQA} > \sqrt{0.52\cdot\mathrm{SWE}}$ implies deep
cooperative phase; $\mathrm{GPQA} < \sqrt{0.52\cdot\mathrm{SWE}}$ implies a new tax is opening.
...
$h_{\rm frontier} = \text{GPQA}_{\rm measured} - (0.52\cdot\text{SWE} + 45.7)$.
...
The cooperative fit enables a forward prediction: $\text{GPQA}_{\rm pred} = 0.52\cdot\text{SWE} + 45.7 + h_{\rm lab}$---improving SWE by 5~pp predicts a 4~pp GPQA gain at no additional compute, and vice versa.
...
(c)~$h$-field for all 31 models. OpenAI trajectory ($h>0$ throughout) vs.\ Anthropic oscillation (Haiku~4.5 and Sonnet~4.6 both dip below; Opus~4.6 recovers). Gemini~2.5~Pro ($h=+8.9$) is the frontier Phi analogue.}
\label{fig:frontier}
\end{figure*}
...
At $N_{c,2}$, the active coupling is $\text{GPQA} = 0.52\cdot\text{SWE} + 45.7$ (the cooperative $h$-field regression).
...
=======
The framework has already survived several internal consistency checks: OLMo at $\gamma_{12}=0.000$ with zero free parameters, Llama-2 holdout at 5.6\% MAE, 12 diagnostics converging on one coupling structure, and monotone signal strengthening across three dataset expansions (8 $\to$ 26 $\to$ 44 $\to$ 63 models).
>>>>>>> Stashed changes
...
\textbf{Extensions beyond LLM scaling.}
The CAPE protocol---compute $\gamma_{12}$ across a scaling axis, locate the sign change---is not specific to model size or to reasoning/truthfulness.
It applies directly to \emph{grokking}~\cite{power2022} (scaling axis = training steps), \emph{multilingual models} (scaling axis = number of training languages; detecting the scale at which the ``curse of multilinguality'' becomes a multilingual bonus), and \emph{modality coupling} in vision-language models ($\gamma_{RV}$ between vision and language benchmark scores).
Each setting introduces its own critical scale $N_c$ with the same GL phase structure; these extensions require no new theoretical machinery---only paired benchmark measurements along the relevant scaling axis.
MoE architectures may complicate the picture: Mistral MoE shows non-monotone coupling ($h = +6, +18, +4$ across three scales), possibly because sparse routing creates an effective parameter count different from the total.
...
Applied to 63 models across 16 families, the central findings are:
...
The cooperative fit provides a forward prediction: $\text{GPQA}_{\rm pred} = 0.52\cdot\text{SWE} + 45.7 + h_{\rm lab}$ (validated at 6.9\% MAE across 4 held-out labs).
...
Applied to all 63 base models: 59/63 correctly classified (94\%).
...
$h_2 = \text{GPQA} - (0.52\cdot\text{SWE} + 45.7) = 88 - (0.52 \times 80 + 45.7) = 88 - 87.3 = +0.7$. Near the cooperative baseline ($|h_2| < 5$): no SWE-GPQA tax excursion. If $h_2 < -10$ (like Sonnet~4.6 at $h=-12.9$), flag it as a coding-specialist vortex before deployment.
