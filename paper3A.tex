...
(Table~\ref{tab:frontier}; scores as of February--March~2026; $h = \text{GPQA}_{\rm measured} - (0.80\cdot\text{SWE} + 24.5)$):
...
Gemini~3.1~Pro has $h = +5.4$ (above the cooperative curve).
The most striking $h$-field excursion is Gemini~2.5~Pro at $h = +11.5$: it substantially overshoots the cooperative regression because Google's training emphasised scientific reasoning before coding optimisation, exactly the frontier-scale analogue of the Phi data-quality effect in the HS-TQA phase.
DeepSeek~V3.2 has $h = -4.1$ (below the cooperative curve, indicating a coding-specialist training recipe that partially trades reasoning performance for coding).
