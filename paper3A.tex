...
The full Arrhenius derivation and phase-boundary surface analysis are in Supplementary Appendix~A.
...
\subsection{Using CAPE in practice}
...
The $\sqrt{}$ isocline classifier generalises to every regime: $\text{score}_2 > \sqrt{(a/b)\cdot\text{score}_1}$ implies cooperative phase; below implies a tax is opening.
This single formula works at $N_{c,1}$ (HS-TQA), $N_{c,2}$ (SWE-GPQA), and $N_{c,3}$ (GPQA-IFEval) without modification.

The implications differ by scale:
\textit{Below $N_{c,1}$ ($N < 3.5$B):} data curation is the primary alignment lever---one unit of quality is worth ${\sim}10\times$ in parameter count (the Phi demonstration).
\textit{At frontier scale ($N_{c,2}$ regime):} track $\gamma_{12}(\text{SWE}, \text{GPQA})$ across consecutive releases; $|h| > 10$ warrants investigation.
The cooperative fit provides a forward prediction validated at 6.9\% MAE across 4 held-out labs.
\textit{At the $N_{c,3}$ frontier (SWE $> 78\%$):} switch to tracking $\gamma_{12}(\text{GPQA}, \text{IFEval})$---the practical lever shifts from coding optimisation to the balance between reasoning depth and instruction compliance.
A fully worked example is given in Appendix~C.

\medskip
\noindent\textbf{Code, data, and dashboard.}\footnote{Scripts: \texttt{cape\_frontier\_full.py}, \texttt{cape\_architecture\_probe.py}, \texttt{cape\_stiffness.py}, \texttt{cape\_algebraic\_nc.py}, \texttt{cape\_family\_hfield.py}, \texttt{cape\_null\_model.py}, \texttt{cape\_phase\_transfer.py}, \texttt{olmo\_gradient\_validation.py}.}
All benchmark data, analysis scripts, and figure-generation code are at \href{https://github.com/adilamin89/cape-scaling}{\texttt{github.com/adilamin89/cape-scaling}}.
The interactive dashboard at \href{https://adilamin89.github.io/cape-scaling}{\texttt{adilamin89.github.io/cape-scaling}} implements the full protocol.
