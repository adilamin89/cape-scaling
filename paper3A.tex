...
\begin{abstract}
Scaling laws describe how aggregate loss improves with model size, but say nothing about how the
capabilities that matter for safe deployment---reasoning, truthfulness, factual accuracy---interact with each other.
Here we show that they conflict below a critical scale, and that this conflict resolves through a sharp, measurable transition.

Below approximately 3.5 billion parameters, reasoning and truthfulness \textbf{anticorrelate} ($r = -0.989$, $p < 10^{-5}$): scaling reasoning actively degrades truthfulness across every web-trained model family we examine---the ``alignment tax.''
Above this scale, the coupling reverses and the two capabilities become cooperative---the ``alignment bonus.''
The transition is confirmed with zero free parameters by OLMo (AI2) at $\gamma_{12}=0.000$, is entirely invisible in loss (featureless power law to 0.8\% precision), and is predicted cross-family at 5.6\% error on held-out Llama-2 data.
\textbf{Two models with identical loss can be in opposite alignment regimes.}

The coupling structure extends far beyond a sign change.
The capability manifold undergoes dimensional collapse ($d_{\rm eff}$ (PCA participation ratio measuring effective capability dimensionality): $2 \to 1$), a Riccati ODE governs the eigenvector rotation through the transition, and a coupled dynamical system discovered via sparse regression reproduces all five benchmark trajectories at 3.6\% error.
Twelve independently measured diagnostics---spanning gradient scaling, eigenvalue geometry, capability susceptibility (how sharply a benchmark responds to scale changes), and geometric phase correlation---converge on one coupling structure parameterised by two numbers.
Three independent universality tests confirm the framework across unseen data: OLMo's zero-parameter confirmation ($\gamma_{12}=0.000$), cross-family hold-out prediction of Llama-2 at 5.6\% error, and eight independent labs converging on $r({\rm HS, TQA})=+0.770$ above $N_c$---a direct measurement of the cooperative regime.

Curated training data eliminates the alignment tax entirely; at one billion parameters, one unit of data quality is worth roughly ten times in model size.
The framework predicts where its own validity ends (near 130 billion parameters) and identifies a testable second transition for frontier capability pairs such as autonomous coding versus safety compliance.
Code and data: \href{https://github.com/adilamin89/cape-scaling}{\texttt{github.com/adilamin89/cape-scaling}}.
\end{abstract}
...
We introduce \textbf{CAPE} (Coupling Analysis of Phase Emergence), a framework that measures how capabilities couple as models scale, and apply it to a simple empirical observation with far-reaching consequences:\footnote{An interactive dashboard with a model-specific phase calculator, all data, and reproduction scripts is available at \url{https://adilamin89.github.io/cape-scaling}.}
...
One might worry that the TQA U-shape reflects evaluation noise rather than a genuine capability shift.
...
\begin{equation}
  ...
  \label{eq:running}
\end{equation}
with zero-crossing at $N_c = 10^{5.886/0.629} \approx 3.5\text{B}$.
...
A more powerful question is whether the coupling leaves a dynamical fingerprint---whether the coupling structure discovered in §\ref{sec:coupling} generates testable predictions about how benchmarks evolve across scale.
...
\emph{jumps 6.3-fold} across the transition (0.12 to 0.75),
...
The quantity $N^{\alpha}(L - E)$ is constant to $\mathrm{CV} = 0.8\%$ across all 8 models from 70M to 12B.
...
Most strikingly, the gradient norm is \emph{non-monotonic}:
...
\caption{\textbf{Gradient scaling.}
(a)~Gradient norm vs model size with power-law fit $\beta = 0.40 \pm 0.08$.
(b)~Loss shows monotonic decrease with no transition visible.
(c)~Gradient vs loss relationship $\|\nabla L\| \propto L^{3.5}$ ($r=0.93$).
(d)~Log-log gradient shows the 1B model dipping
37\% below the power-law trend, exactly within the $N_c$ transition region
...
When a system changes phase, not just one quantity changes---the entire geometry of the state space reorganises.
...
\textbf{Soft mode.}
PCA of the Pythia correlation matrix at cumulative scales reveals a dominant eigenvalue $\lambda_1$ that grows from 3.94 (410M) to 4.48 (12B), and a second eigenvalue $\lambda_2$ that \emph{decreases} from 1.06 to 0.40, well-fit by $\lambda_2 \sim N^{-0.72}$ ($R^2 = 0.95$).
...
with an ODE capturing the geometric trend of a noisy empirical trajectory; the key structural claim is the
...
Between $N_c$ and $N_{c,2}$, different model families---or different training recipes within a family---can sit in locally different coupling regimes before the cooperative phase fully stabilises.
...
the more sensitive benchmarks are to scale changes (high susceptibility $\chi_2$), the faster the coupling runs---a self-reinforcing feedback loop above $N_c$.\footnote{This mechanism is directly analogous to spin-fluctuation-mediated inter-band coupling in heavy-fermion superconductors~\cite{amin2020}, where susceptibility fluctuations renormalize the coupling super-linearly. The mathematical structure is identical. We emphasize this classification is an \emph{analogical} mapping that respects the symmetry structure, not a claim of microscopic equivalence: AI scaling is not a superconductor.}
...
A further consistency check: the SFEE slope $A = 0.629$ equals $\sin(\theta^*) = \sin(38.8^{\circ}) = 0.626$ to within measurement uncertainty---confirming that the self-reinforcing feedback terminates at the phase-locked eigenvector angle, not arbitrarily, and linking the two independent measurements (SFEE correlation in \S\ref{sec:topology} and eigenvector rotation in \S\ref{sec:topology}) into a single identity.\footnote{In superconductor language, this is the TRSB stabilisation condition: the coupling $\gamma_{12}$ locks at the value $\sin\theta^*$ of the relative phase between the two order-parameter components, exactly as derived in the SFEE framework for heavy-fermion superconductors~\cite{amin2020}, where susceptibility fluctuations lock the inter-band phase at the coupling fixed point. The full free energy above $N_c$ includes a stabilising term $-\eta\,\mathrm{Im}(\psi_R^*\psi_C)$, where $\eta \propto \sin\theta^*$; this term is absent below $N_c$ and is what makes the cooperative phase a true free-energy minimum rather than a saddle point.}

\textbf{Overconstrained validation.}
The twelve quantities---$\alpha$, $\gamma_{12}$, $\beta$, the ODE coefficients, $\chi_{ND}$, $h(\mathcal{D})$, $W$, eigenvector rotation, soft-mode $\lambda_2$, gradient anomaly, curvature peak, and geometric phase correlation $r(\gamma_{12},\theta)=+0.47$---are independent measurements from one coupling structure, parameterised by two numbers ($A = 0.629$, $B = -5.886$ in $\gamma_{12}(N) = A\log_{10}N + B$).
...
(b)~Eigenvalue phase portrait: $\lambda_1$ grows while $\lambda_2$ (soft mode (the benchmark-space direction whose variance vanishes at the transition): the eigenvector whose variance shrinks fastest) collapses.
...
\begin{equation}
  h_c(N) \propto (N_c - N)^{3/2} \quad\text{for } N < N_c\footnote{The exponent 3/2 emerges from the cubic GL potential with an external field; standard Landau theory without field gives the mean-field exponent 1/2.}
  \label{eq:design}
\end{equation}
...
(measured as the difference in size-scaling exponent between Chinchilla at optimal $D$ and Pythia at fixed $D$).
...
that the sign change is unique to the reasoning--truthfulness axis.
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=\textwidth]{fig7_frontier.png}
...
With only $n=10$ frontier models and $p=0.34$, this correlation is suggestive rather than conclusive; we present it as a directional signal awaiting larger frontier datasets.
Results are consistent with cooperative coupling at frontier scale (Fig.~\ref{fig:frontier}).
...
The current two-dimensional coupling analysis ($d_{\rm eff} = 2$)
is an effective theory valid in a specific regime.
When $d_{\rm eff}$ increases---detectable from the eigenvalue spectrum---the
...
\begin{enumerate}[leftmargin=1.5em,itemsep=4pt]
  ...
  \item \textbf{The capability manifold restructures at the transition.}
    ...
    and twelve diagnostics (projections of a single two-parameter signal)---gradient scaling, eigenvalue geometry,
    thermodynamic susceptibility (capability sensitivity to scale), geometric phase correlation---all converge
    on one coupling structure parameterised by two numbers.
    The Hessian determinant predicts theory breakdown at $N \approx 130$B,
    making CAPE self-limiting by design.
...
The live interactive dashboard---including a model-specific phase calculator, frontier coupling explorer, and all thermodynamic observables---is hosted at
\href{https://adilamin89.github.io/cape-scaling}{\texttt{adilamin89.github.io/cape-scaling}}.
