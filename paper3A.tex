...
A second capability axis transition is predicted at $N_{c,3} \approx 114$B, where IFEval becomes the last discriminating benchmark in the current generation.
...
A predicted consequence: any model trained at exactly $N_c$ will show minimum gradient norm and maximum susceptibility to alignment interventions.
...
$N_{c,3} \approx 114$--$130$B (predicted: SWE/GPQA/IFEval manifold itself saturates, third axis activates).
Each transition is independently detectable: $N_{c,1}$ by OLMo ($\gamma_{12}=0.000$) and OPT (within-family sign flip); $N_{c,2}$ by the frontier cooperative coupling ($r=+0.85$); $N_{c,3}$ by two-method convergence ($\det(H)\to 0$ and frontier $3{\times}3$ eigenvalue analysis).
...
The \textbf{remaining testable prediction}: the IFEval scores used in the $3{\times}3$ frontier analysis (\S\ref{sec:Nc2_section}) are approximate estimates; confirming them with precise published measurements at matched compute, and then measuring the sign of $\gamma(\text{IFEval}, \text{HarmBench})$ across model generations, is the concrete next test for $N_{c,2}$.
The preliminary evidence (r(SWE, IFEval) $\approx +0.76$ with approximate scores) is consistent with cooperative coupling---but approximate IFEval scores introduce enough uncertainty that the sign of this specific coupling cannot yet be declared confirmed.

\textbf{Predicting the $N_{c,3}$ benchmark axis.}
The frontier $3{\times}3$ correlation matrix (SWE-bench, GPQA Diamond, IFEval) has eigenvalues $\lambda_1=2.16$, $\lambda_2=0.67$, $\lambda_3=0.17$ and $\det(\Gamma)=0.24$.
Extrapolating $\det(\Gamma)\to 0$ from the frontier data gives $N_{c,3} \approx 114$B---consistent with the $\sim$130B prediction from Pythia's $\det(H)\to 0$.
Two-method convergence on $N_{c,3} \approx 110$--130B suggests this is robust.
...
When SWE-bench and GPQA Diamond saturate---as HellaSwag and TruthfulQA saturated before them---IFEval will be the last discriminating benchmark in the current generation.
...
We recommend frontier labs begin tracking $\gamma(\text{IFEval}, \text{HarmBench})$ now---if it turns negative, $N_{c,3}$ is within reach.
...
Two-method convergence (Pythia $\det(H)$ extrapolation and frontier $3{\times}3$ eigenvalue analysis) predicts $N_{c,3} \approx 114$--$130$B, where the SWE/GPQA/IFEval manifold itself exhausts $d_{\rm eff}\to 3$.
IFEval dominates the primary eigenvector at this scale ($\lambda_1$ loading $= -0.64$), making it the last discriminating benchmark in the current generation.
We recommend frontier labs begin tracking $\gamma(\text{IFEval}, \text{HarmBench})$ now: this pair operationalizes the question of whether a model can complete complex autonomous tasks without unsafe behavior---the natural next axis of competition.
When this coupling turns negative, the field will need a new benchmark designed to be orthogonal to IFEval; Paper~3B develops the eigenvector rotation formalism for detecting this in advance.
