...
Across 19 frontier models spanning six labs (2024--2026), SWE-bench and GPQA Diamond couple cooperatively ($r=+0.85$, $p<10^{-5}$), with training-recipe excursions visible as measurable $h$-field deviations.
...
The subscript convention follows the order of discovery; the physics at each transition is the same---a coupling sign change between the active capability pair.
...
are the strongest Nc3-detection candidates based on this criterion.
...
The concrete next test for $N_{c,2}$ is measuring $\gamma(\text{IFEval}, \text{HarmBench})$ across consecutive releases (\S\ref{sec:Nc2_section}).
